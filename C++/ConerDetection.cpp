...
int connerDetection(ResultLines *resultLines, Point vanPoint, Size image ,Size range, double k)
{
    ...
    int result = nomalConner(resultLines,vanPoint);
    result = isVerticalConner(resultLines,vanPoint);
...
int connerDetection(vector<Point> right_point, vector<Point> left_point, double k)
{
    ...
    int right_size = right_point.size(), left_size=left_point.size();
//    cout << "right CPoint :" << right_size << "  left CPoint :"<<left_size;

    int bigSize=right_size, smallSize =left_size; bool isbig_right=true;
    if(right_size<left_size){
        bigSize = left_size;
        smallSize = right_size;
        isbig_right = false;
    }

    /**
     * definition of constant value K
     * k는 소실점 주위의 교점의 갯수를 구하고, 왼쪽 또는 오른쪽의 교점의 갯수가 일정 비율 이상인지를 감지하는
     */
    if((bigSize-smallSize)/(bigSize+smallSize*1.0)>k)
    {
        if(isbig_right)
            return 1;
        else
